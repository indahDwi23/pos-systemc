        ...
        <form action="/menu" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select class="form-select @error('category') is-invalid @enderror" id="category" name="category" required>
                    ...
                    <option value="food"  @if (old('category') == "food") {{ 'selected' }} @endif >Makanan</option>
                    <option value="drink"  @if (old('category') == "drink") {{ 'selected' }} @endif >Minuman</option>
                </select>
                @error('category')
                    <div class="invalid-feedback">
                        {{ $message }}
            ...
            <div class="mb-3">
                <label class="form-label" for="img">Unggah Gambar</label>
                @error('image')
                    <p class="small text-danger">{{ $message }}</p>
                @enderror
                <div class="img-area">
                    <img class="img-preview" style="width: 100%; height: 240px; object-fit: cover; border-radius: 10px;">
            ...
            <div class="mb-4">
                <button class="btn-submit btn btn-primary" type="submit">Tambah Menu</button>
            </div>
        </form>
    </div>
    <script>
        ...
        img.addEventListener('change', function() {
            ...
            fileReader.addEventListener("load", function () {
                ...
                view_path_img.innerHTML = ` <h3>${files.name}</h3> <p>klik untuk mengubah</p>`;
